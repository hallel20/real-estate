...
const PropertyDetailPage: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
        ...
        <div>
          <div className="flex items-center mb-2">
            <span className={`inline-block px-2 py-1 text-xs font-semibold rounded mr-2 ${
              currentProperty.status === 'for-sale' ? 'bg-blue-600 text-white' : 'bg-green-600 text-white'
            }`}>
            ...
            <span className={`inline-block px-2 py-1 text-xs font-semibold rounded bg-gray-200 text-gray-800`}>
              {currentProperty.propertyType.charAt(0).toUpperCase() + currentProperty.propertyType.slice(1)}
            </span>
          </div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">{currentProperty.title}</h1>
          <div className="flex items-center text-gray-600">
            <MapPin className="h-5 w-5 mr-1" />
            <span>{currentProperty.location.address}, {currentProperty.location.city}, {currentProperty.location.state} {currentProperty.location.zipCode}</span>
          </div>
        </div>
      ...
      {/* Property Gallery */}
      <div className="mb-8">
        <PropertyGallery images={currentProperty.images} title={currentProperty.title} />
      </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
                ...
                <span className="text-lg font-semibold">{currentProperty.features.bedrooms}</span>
              </div>
              <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
                <Bath className="h-6 w-6 text-blue-600 mb-2" />
                <span className="text-sm text-gray-600">Bathrooms</span>
                <span className="text-lg font-semibold">{currentProperty.features.bathrooms}</span>
              </div>
              <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
                <Square className="h-6 w-6 text-blue-600 mb-2" />
                <span className="text-sm text-gray-600">Area</span>
                <span className="text-lg font-semibold">{currentProperty.features.area.toLocaleString()} sqft</span>
              </div>
              <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
                <Calendar className="h-6 w-6 text-blue-600 mb-2" />
                <span className="text-sm text-gray-600">Year Built</span>
                <span className="text-lg font-semibold">{currentProperty.features.yearBuilt || 'N/A'}</span>
              </div>
            </div>
            
            <div className="border-t border-gray-200 pt-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Description</h3>
              <p className="text-gray-700 mb-6 whitespace-pre-line">{currentProperty.description}</p>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div className="aspect-video bg-gray-200 rounded-lg mb-4">
              ...
              <div className="w-full h-full flex items-center justify-center">
                ...
                <span className="text-gray-600">Map view would be displayed here</span>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mt-6">
            ...
            <ul className="space-y-3">
              ...
              <li className="flex justify-between">
                ...
                <span className="font-medium">{currentProperty.propertyType.charAt(0).toUpperCase() + currentProperty.propertyType.slice(1)}</span>
              </li>
              <li className="flex justify-between">
                <span className="text-gray-600">Status:</span>
                <span className="font-medium">{currentProperty.status === 'for-sale' ? 'For Sale' : 'For Rent'}</span>
              </li>
              {currentProperty.features.yearBuilt && (
                <li className="flex justify-between">
                  <span className="text-gray-600">Year Built:</span>
                  <span className="font-medium">{currentProperty.features.yearBuilt}</span>
                </li>
              )}
              {currentProperty.features.parking !== undefined && (
                <li className="flex justify-between">
                  <span className="text-gray-600">Parking:</span>
                  <span className="font-medium">{currentProperty.features.parking} spaces</span>
                </li>
              )}
              <li className="flex justify-between">
                <span className="text-gray-600">Listed:</span>
                <span className="font-medium">{formatDate(currentProperty.createdAt)}</span>
              </li>
              <li className="flex justify-between">
                <span className="text-gray-600">Last Updated:</span>
                <span className="font-medium">{formatDate(currentProperty.updatedAt)}</span>
              </li>
            </ul>
          </div>
        </div>
